...
Group names containing a comma, pipe or backslash use the same escaping in all processing stages. For example, the shell argument `--sample_group 'brain\,adult,liver\|part'` selects two groups. `order` and `highest_ties` use escaped pipe-separated lists; the Python helper `amalgkit.text_utils.parse_sample_group_argument` decodes them. `highest` is a single literal group name. Duplicate or empty tissue columns are rejected by the tau calculation.

The Python helper `sample_group_to_tau` accepts linear representatives only; its former `transform_method` argument has been removed to prevent accidental inverse transformation after averaging. Use `linear_sample_group_summary` to decode run expression and aggregate it first. The log2p1 inverse uses `expm1` to preserve tiny positive values, and finite means are scaled during accumulation to avoid overflow.
...
Tau panels retain the supplied run metadata even when other QC panels intersect available runs. Missing runs therefore make tau undefined instead of silently changing replicate weights. Before/after tau panels summarize each supplied matrix separately, while correlation panels use their common runs.
