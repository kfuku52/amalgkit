...
config = read_amalgkit_config('wsfilter.r')
...
robust_margin_threshold = as.numeric(config[['robust_margin_threshold']])
robust_z_threshold = as.numeric(config[['robust_z_threshold']])
...
cat('outlier_method:', outlier_method, "\n")
...
check_within_sample_group_correlation = function(tc, sra, dist_method, min_dif, selected_sample_groups, one_out_per_iter = TRUE, correlation_threshold) {
    if (outlier_method == 'robust_margin') {
        ...
        coef_matrix_all = as.matrix(suppressWarnings(cor(tc, tc_ave_all, method = dist_method, use = 'pairwise.complete.obs')))
        if (is.null(dim(coef_matrix_all))) {
            coef_matrix_all = matrix(coef_matrix_all, nrow = ncol(tc), ncol = ncol(tc_ave_all))
        }
        if (is.null(rownames(coef_matrix_all))) {
            rownames(coef_matrix_all) = colnames(tc)
        }
        if (is.null(colnames(coef_matrix_all))) {
            colnames(coef_matrix_all) = colnames(tc_ave_all)
        }
        sra2[['ws_within_group_cor']] = NA_real_
        sra2[['ws_max_nongroup_cor']] = NA_real_
        sra2[['ws_margin']] = NA_real_
        for (run_id in colnames(tc)) {
            run_idx = which(as.character(sra2[['run']]) == as.character(run_id))
            if (length(run_idx) == 0) {
                next
            }
            run_idx = run_idx[[1]]
            if (!run_id %in% rownames(coef_matrix_all)) {
                next
            }
            my_group = as.character(sra2[['sample_group']][run_idx])
            coef_row = coef_matrix_all[run_id, selected_sample_groups, drop = TRUE]
            if (length(coef_row) == 0) {
                next
            }
            if (is.null(names(coef_row))) {
                names(coef_row) = selected_sample_groups
            }
            if (!my_group %in% names(coef_row)) {
                next
            }
            within_cor = suppressWarnings(as.numeric(coef_row[[my_group]]))
            other_names = setdiff(names(coef_row), my_group)
            other_vals = suppressWarnings(as.numeric(coef_row[other_names]))
            other_vals = other_vals[is.finite(other_vals)]
            max_nongroup = if (length(other_vals) == 0) NA_real_ else max(other_vals)
            margin_val = ifelse(is.finite(within_cor) & is.finite(max_nongroup), within_cor - max_nongroup, NA_real_)
            sra2[['ws_within_group_cor']][run_idx] = within_cor
            sra2[['ws_max_nongroup_cor']][run_idx] = max_nongroup
            sra2[['ws_margin']][run_idx] = margin_val
        }
        sra2 = flag_margin_outliers(
            df = sra2,
            margin_col = 'ws_margin',
            group_col = 'sample_group',
            margin_threshold = robust_margin_threshold + min_dif,
            robust_z_threshold = robust_z_threshold,
            robust_z_col = 'ws_robust_z',
            outlier_col = 'ws_outlier_candidate'
        )
        exclude_runs = unique(as.character(sra2[['run']][sra2[['ws_outlier_candidate']] %in% TRUE]))
        exclude_runs = exclude_runs[(!is.na(exclude_runs)) & (exclude_runs != '')]
        if (one_out_per_iter && (length(exclude_runs) > 0)) {
            candidate_df = sra2[sra2[['run']] %in% exclude_runs, c('run', 'sample_group', 'bioproject')]
            candidate_df = candidate_df[!is.na(candidate_df[['run']]) & (candidate_df[['run']] != ''), , drop = FALSE]
            first_bp = candidate_df[match(unique(candidate_df[['bioproject']]), candidate_df[['bioproject']]), 'run']
            first_group = candidate_df[match(unique(candidate_df[['sample_group']]), candidate_df[['sample_group']]), 'run']
            exclude_runs = unique(as.character(c(first_bp, first_group)))
            exclude_runs = exclude_runs[(!is.na(exclude_runs)) & (exclude_runs != '')]
            cat('Excluding one outlier per BioProject/sample_group in this iteration.\n')
        }
        if (length(exclude_runs) > 0) {
            cat('Removed Runs due to low within-sample_group robust margin score:', paste(exclude_runs, collapse = ' '), '\n')
        }
        tc = tc[, !colnames(tc) %in% exclude_runs, drop = FALSE]
        sra[(sra[['run']] %in% exclude_runs), 'exclusion'] = 'low_within_sample_group_correlation'
        metric_cols = c('ws_within_group_cor', 'ws_max_nongroup_cor', 'ws_margin', 'ws_robust_z', 'ws_outlier_candidate')
        run_idx_map = match(as.character(sra[['run']]), as.character(sra2[['run']]))
        for (metric_col in metric_cols) {
            if (!metric_col %in% colnames(sra)) {
                sra[[metric_col]] = NA
            }
            sra[[metric_col]] = sra2[[metric_col]][run_idx_map]
        }
        return(list(tc = tc, sra = sra))
    }
    if (length(selected_sample_groups) == 1) {
...
draw_boxplot = function(sra, tc_dist_matrix, fontsize = 8) {
    ...
    par(
        mar = c(max(old_mar[1], 6.2), max(old_mar[2], 6.2), old_mar[3], old_mar[4]),
        mgp = c(2.8, 0.8, 0)
    )
    ...
    if (length(finite_idx) > 0) {
        points(group_positions[finite_idx], means[finite_idx], col = "red", pch = 16)
    }
