...
draw_sva_summary = function(sva_out, tc, sra, fontsize) {
    if ((is.null(sva_out)) | (class(sva_out) == "try-error")) {
        ...
        sra = out[["sra"]]
        # cols =
        # c('tissue','bioproject','lib_selection','layout','instrument','num_read_masked','fraction_lost_fastp',
        # 'fraction_lost_mask','min_read_len_masked','avg_read_len_masked','max_read_len_masked','mapping_rate')
        cols = c("tissue", "bioproject", "lib_selection", "instrument", "num_read_fastp", "mapping_rate")

        # label_cols = c('organ','BioProject','library selection','library layout','instrument','number of
        # read','% lost, fastp', '% lost, misc feature','minimum read length','average read length','maximum
        # read length','mapping rate')
...
# Quick workaround for missing num_read_fastp.  When activated, all samples pass the mapping rate
# cutoff Remove this codeblock when fixed.
if (FALSE) {
    print(dim(tc))
